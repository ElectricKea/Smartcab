        ...
        if self.learning == True:
            if not state in self.Q:   
                for action in self.valid_actions:
                    action_list[action] = 0.0
                self.Q[state] = action_list
...
def run():
    ...
    sim = Simulator(env, log_metrics=True, display=False, update_delay=0.001, optimized=True)
    ...
    sim.run(n_test=75, tolerance=0.01)
